---
name: markitdown
description: Convert files to Markdown — PDF, DOCX, PPTX, XLSX, images (OCR), audio/video (transcription), HTML, CSV, JSON, XML, ZIP, EPUB, YouTube URLs. WHEN: "convert to markdown", "extract text from PDF", "transcribe audio", "markitdown this file".
license: MIT
metadata:
  author: KANINI
  version: 1.0.0
...
| | |
|---|---|
| **Best for** | Converting files to Markdown for LLM processing |
| **Primary capabilities** | 15+ formats · OCR · speech transcription · AI image descriptions · Azure Document Intelligence |
| **Scripts** | `scripts/batch_convert.py` · `scripts/convert_literature.py` · `scripts/convert_with_ai.py` |
...
|---|---|---|
| File not found | `FileNotFoundError` | Verify the file path; ask user if unclear |
| Missing optional dependency | "Install markitdown[pdf]" etc | Install the feature-specific pip extra |
| Binary read error | — | Open in binary mode (`"rb"`) when using `convert_stream` |
| OCR failure | "tesseract not found" | Install tesseract (`brew install tesseract` / `apt install tesseract-ocr`) |
| AI description cost | — | Confirm OpenRouter key set; monitor usage |
