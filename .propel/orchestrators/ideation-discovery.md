...
Ideation on-ramp into `technical-discovery`: brainstorm ideas, create a
specification, gate on approval of the spec, then hand off for architecture
design. Replaces `discovery-lite` (which stopped at an approved brainstorm
and let `technical-discovery` create the spec) -- `technical-discovery` no
longer creates the spec itself, so this workflow now owns that step.
...
Build `pathContext = {"brainstorm": {...}, "spec": {...}}` (same
`{path, content_type, mcp_type, quality_threshold}` shape as other orchestrators). Read
...
```
StartWorkflowRun(workflow="ideation-discovery", version="1.0", project=<projectName>,
...
request cannot lower it. S1 (`brainstorm-idea`) carries no gate at all;
completing it advances straight into S2.

## Step 2 — execute S1 inline (brainstorm-idea)
...
1. `GetNextRunStep(project, runId)` → fetch directive
2. Execute spec creation inline
...
- `propel-ideation-discovery-s1`/`s2` subagents must never call
