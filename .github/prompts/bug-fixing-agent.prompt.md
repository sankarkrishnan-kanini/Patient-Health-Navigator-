---
agent: agent
description: For Developers. Orchestrates bug resolution phase—triage, test impact analysis, implementation, analysis, UX review (if UI), code review, test updates, and full test suite validation.
tools: ['execute/createAndRunTask', 'read/readFile', 'azure-mcp/search', 'edit/createDirectory', 'edit/createFile', 'edit/editFiles', 'search', 'web', 'todo', propel-sdlc/*]
---

consult `.propel/orchestrators/bug-fixing-agent.md` for the workflow steps.

---

*This agent orchestrates bug resolution for Software Developers and QA Engineers.*
