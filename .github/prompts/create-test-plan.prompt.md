---
description: Generates comprehensive E2E / feature specific test plan requirements covering functional, NFR, technical, and data validations.
tools: ['execute/getTerminalOutput', 'execute/createAndRunTask', 'execute/runInTerminal', 'read/problems', 'read/readFile', 'edit/createDirectory', 'edit/createFile', 'edit/editFiles', 'search', 'web', 'azure-mcp/search', 'sequential-thinking/*', 'todo', propel-sdlc/*]
---

consult `.propel/prompts/create-test-plan.md` for the workflow steps.

---

*This generator creates comprehensive test plans with FR/NFR/TR/DR coverage and traceability.*
